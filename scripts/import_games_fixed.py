#!/usr/bin/env python3
"""
Fixed Game Import Script for AWS Server
Handles all 2000+ games with proper error handling
"""
            ...
            for row_num, row in enumerate(csv_reader, start=1):
                try:
                    ...
                    if not game_id or not game_name:
                        print(f"⚠️  Skipping row {row_num}: Missing game_id or game_name")
                        error_count += 1
                        continue
                    
                    # Create internal name from game name
                    internal_name = game_name.lower().replace(' ', '_').replace('-', '_')
