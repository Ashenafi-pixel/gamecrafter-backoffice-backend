...
import (
	...
	"github.com/tucanbit/internal/storage/operator"
	...
	"github.com/tucanbit/internal/storage/player"
	"github.com/tucanbit/internal/storage/provider"
...
func initPersistence(persistencdb *persistencedb.PersistenceDB, log *zap.Logger, gormDB *gorm.DB, redis *redis.RedisOTP, userWS utils.UserWS, clickhouseClient *clickhouse.ClickHouseClient) *Persistence {
	...
	return &Persistence{
		...
		Player:               player.Init(persistencdb, log),
		Provider:             provider.Init(persistencdb, log),
		CryptoWallet:         storage.Init(persistencdb, log),
		Report:               report.Init(persistencdb, analyticsStorageInstance, log),
		...
		Operator: operator.NewOperatorStorage(persistencdb, log),
